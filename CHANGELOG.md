...
- Pin shared npm CI/CD component to `@v1.2.2` — fixes GitLab Release creation, which was 404ing on the wrong auth header (`PRIVATE-TOKEN` instead of `JOB-TOKEN`).

## [2.2.14] - 2026-08-23

### Changed

- Pin shared npm CI/CD component to `@v1.2.1` — fixes the GitLab Release-creation step, which was silently failing (`curl: not found`) on every prior release.
