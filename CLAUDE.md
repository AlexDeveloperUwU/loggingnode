...
- **Auto-detection**: service name, version, commit hash, region, and instance ID resolve automatically (options → env vars → `package.json` / CI variables / `os.hostname()` → `'unknown'`). Zero-config is the common case; nothing throws for missing context.
...
│   ├── context.js          # AsyncLocalStorage + wide-event lifecycle (startEvent/enrichEvent/endEvent/withContext)
│   ├── streams.js          # pino.multistream assembly: pretty / JSON stdout / pino-seq
│   ├── redaction.js        # DEFAULT_REDACT_PATHS + user-path merging
│   ├── serializers.js      # err/error serializers → { type, message, code, stack }
│   ├── sampling.js         # tail-sampling hook evaluation
...
└── docker-compose.seq.yml  # optional local Seq for development
```
...
- **Naming:** camelCase for JS identifiers (variables, functions, options), but **snake_case for all log field names** (`request_id`, `duration_ms`, `commit_hash`). This split is deliberate and is the #1 source of confusion in this codebase — log fields are a cross-service schema, code style is not.
...
- **Message prefix** is applied via pino's `hooks.logMethod` (rewrite the string argument, never mutate the merging object). Do not try to do it in `formatters.log` — `msg` is not reliably visible there. Never bypass the hook by writing to destinations directly.
- **Redaction censor is a function** implementing the 75/25 rule: values shorter than 3 chars → `***`; otherwise mask all but the **last** 25% with `*`. Keep it deterministic, total (any input type → string), and allocation-cheap — it runs per matched field, per event.
...
- **NEVER throw for missing configuration that has a fallback.** Auto-detection (package.json, CI env vars, hostname) ends at `'unknown'`/`[UNK]` plus one stderr warning. Exceptions are reserved for programmer errors (e.g. invalid option types).
...
- The `[APP] · ` message tag is implemented in `hooks.logMethod(args, method, level)`: find the first string argument, prefix it, then `method.apply(this, args)`. The tag derives from the resolved service name (3 chars, uppercase, `*`-padded, `UNK` fallback) and is computed once at `createLogger` time.
- pino's `redact` accepts `censor` as a function `(value, path) => string` — the library default implements the 75/25 proportional mask there. A user-supplied string or function replaces it wholesale.
- Auto-detection reads, in order: explicit option, documented env var, then filesystem/runtime (`package.json` walked up from `process.cwd()`, `CI_COMMIT_SHA`/`GITHUB_SHA`, `AWS_REGION`/`FLY_REGION`, `os.hostname()`). Keep `resolveConfig` pure — the impure `detectContext()` (package.json + hostname) is called once in `createLogger` and its result passed in.
